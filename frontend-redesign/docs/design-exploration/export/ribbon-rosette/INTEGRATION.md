...
La rosette dont chaque boucle est ombrée sur sa longueur : les pétales lisent comme du ruban plié qui se soulève.
...
2. **Fichier statique** — `<img src="/brand/ribbon-rosette/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Rotation continue très lente (18 s) plus un décalage d'opacité qui donne l'impression du ruban qui tourne sur lui-même.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-rosette">…</svg>
...
```text
Intègre le logo SALI AI (variante 5C « Ribbon rosette ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, ribbon-rosette/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de ribbon-rosette/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : ribbon-rosette/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
