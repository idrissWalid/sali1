...
Le nom EST le logo : lettrage arrondi, « SALI » dans une pilule d'accent, « AI » libre à côté. La pilule se recadre en badge « S » pour l'icône.
...
La pilule s'étire depuis la gauche (scaleX 0→1, origine gauche) puis « AI » entre en fondu décalé de 180 ms.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-pill">…</svg>
...
```text
Intègre le logo SALI AI (variante 2A « Wordart pill ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, wordart-pill/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de wordart-pill/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
