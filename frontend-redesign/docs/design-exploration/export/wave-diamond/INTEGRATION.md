...
Un losange plein traversé de deux rainures ondulées : le signal qui passe à travers la masse de données.
...
2. **Fichier statique** — `<img src="/brand/wave-diamond/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Le losange grandit d'un quart de tour, puis les deux ondes glissent en boucle — le mouvement reste clippé à l'intérieur de la forme.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-waves">…</svg>
...
```text
Intègre le logo SALI AI (variante 3E « Wave diamond ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, wave-diamond/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de wave-diamond/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : wave-diamond/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
