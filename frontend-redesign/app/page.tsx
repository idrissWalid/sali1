...
import { Panel, Group, Separator, usePanelRef, type PanelSize } from "react-resizable-panels";
...
export default function Home() {
  ...
  const syncLeftPanelState = useCallback((size: PanelSize) => {
    setIsLeftPanelOpen(size.inPixels > COLLAPSED_PANEL_SIZE + 1);
  }, []);

  const syncStudioPanelState = useCallback((size: PanelSize) => {
    setIsStudioPanelOpen(size.inPixels > COLLAPSED_PANEL_SIZE + 1);
  }, []);
    ...
    <div className="app-shell">
      ...
      <main id="main-workspace" className="app-workspace" style={{ display: "flex", alignItems: "center" }}>
        <Group orientation="horizontal" className="workspace-panels" style={{ width: "100%", height: "98%", margin: "auto" }}>
          <Panel panelRef={leftPanelRef} onResize={syncLeftPanelState} defaultSize="22%" minSize="15%" collapsible collapsedSize={`${COLLAPSED_PANEL_SIZE}px`} className={`workspace-panel workspace-panel--data${isLeftPanelOpen ? "" : " is-collapsed"}${mobileView === "data" ? " is-mobile-active" : ""}`} style={{ height: "100%" }}>
            <div style={{
              height: "100%",
              display: "flex",
          ...
          <Separator className="workspace-separator" style={{ width: "8px" }}>
            <button
              ...
              aria-label={isLeftPanelOpen ? "Fermer le panneau Données" : "Ouvrir le panneau Données"}
              aria-expanded={isLeftPanelOpen}
              title={isLeftPanelOpen ? "Réduire le panneau Données" : "Afficher le panneau Données"}
            >
          ...
          <Separator className="workspace-separator" style={{ width: "8px" }}>
            <button
              ...
              aria-label={isStudioPanelOpen ? "Fermer le panneau Studio" : "Ouvrir le panneau Studio"}
              aria-expanded={isStudioPanelOpen}
              title={isStudioPanelOpen ? "Réduire le panneau Studio" : "Afficher le panneau Studio"}
            >
          ...
          <Panel panelRef={studioPanelRef} onResize={syncStudioPanelState} defaultSize="23%" minSize="15%" collapsible collapsedSize={`${COLLAPSED_PANEL_SIZE}px`} className={`workspace-panel workspace-panel--studio${isStudioPanelOpen ? "" : " is-collapsed"}${mobileView === "studio" ? " is-mobile-active" : ""}`} style={{ height: "100%" }}>
