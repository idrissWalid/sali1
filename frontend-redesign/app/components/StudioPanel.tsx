...
import { AudioLines, BrainCircuit, FileText, ImagePlus, LayoutDashboard, Loader2, LineChart, Presentation, X } from "lucide-react";
...
type ReportLogo = { name: string; dataUrl: string };
...
export default function StudioPanel({ sessionId, generatedContent, chatModelPending = false, modelsRefreshKey = 0 }: Props) {
  ...
  const downloadReport = async (format: "pdf" | "word" | "powerpoint", keyPoints = "", logoDataUrl?: string) => {
    if (!sessionId) {
  ...
  const handleReportLogoChange = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.currentTarget.files?.[0];
    event.currentTarget.value = "";
    if (!file) return;
    if (!(["image/png", "image/jpeg"] as string[]).includes(file.type)) {
      alert("Utilisez un logo PNG ou JPEG.");
      return;
    }
    if (file.size > 3 * 1024 * 1024) {
      alert("Le logo doit peser moins de 3 Mo.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      if (typeof reader.result === "string") {
        setReportLogo({ name: file.name, dataUrl: reader.result });
      }
    };
    reader.onerror = () => alert("Le logo n'a pas pu être lu.");
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      await downloadReport(reportFormat, reportKeyPoints, reportLogo?.dataUrl);
      ...
      <Modal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} title="Générer un rapport" maxWidth="560px">
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <p style={{ margin: 0, color: "var(--text-muted)", fontSize: "13px" }}>
            Précisez l’objectif du livrable, choisissez son format et ajoutez éventuellement votre logo.
          </p>
          <label htmlFor="report-key-points" style={{ fontSize: "12px", fontWeight: 650, color: "var(--text-main)" }}>
            Angle et points à mettre en avant <span style={{ color: "var(--text-muted)", fontWeight: 500 }}>(facultatif)</span>
          </label>
          <div style={{ position: "relative" }}>
            <textarea
              id="report-key-points"
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ fontSize: "12px", fontWeight: 650, color: "var(--text-main)" }}>
              Logo de couverture <span style={{ color: "var(--text-muted)", fontWeight: 500 }}>(facultatif)</span>
            </div>
            <input id="report-logo" className="sr-only" type="file" accept="image/png,image/jpeg" onChange={handleReportLogoChange} />
            <div style={{ display: "flex", alignItems: "center", gap: "8px", minWidth: 0 }}>
              <label htmlFor="report-logo" style={{ minHeight: "44px", flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: "9px", padding: "10px 12px", border: "1px dashed var(--border-color)", borderRadius: "10px", background: "var(--bubble-ai)", color: "var(--text-main)", cursor: "pointer" }}>
                <ImagePlus size={18} aria-hidden="true" style={{ flex: "0 0 auto", color: "var(--action-primary)" }} />
                <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px", fontWeight: 600 }}>
                  {reportLogo ? reportLogo.name : "Ajouter un logo PNG ou JPEG"}
                </span>
              </label>
              {reportLogo && (
                <button type="button" onClick={() => setReportLogo(null)} aria-label="Retirer le logo" title="Retirer le logo" style={{ width: "44px", height: "44px", flex: "0 0 44px", display: "grid", placeItems: "center", border: "1px solid var(--border-muted)", borderRadius: "10px", background: "var(--bubble-ai)", color: "var(--text-muted)" }}>
                  <X size={17} aria-hidden="true" />
                </button>
              )}
            </div>
            <div style={{ color: "var(--text-muted)", fontSize: "11px", lineHeight: 1.45 }}>
              3 Mo maximum. Le logo apparaît uniquement sur la couverture.
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              {FORMATS_RAPPORT.map((f) => (
                <button
                  ...
                  style={{ minHeight: "44px", flex: "1 1 120px", padding: "10px 12px", borderRadius: "10px", border: reportFormat === f.id ? "1px solid var(--accent-color)" : "1px solid var(--border-muted)", background: reportFormat === f.id ? "var(--accent-soft)" : "var(--bubble-ai)", color: "var(--text-main)", fontWeight: 600 }}
                >
                  {f.label}
                </button>
              ))}
          ...
          <button
            ...
            style={{ minHeight: "44px", padding: "11px 14px", borderRadius: "10px", border: "none", background: "var(--action-primary)", color: "white", fontWeight: 700, cursor: (!sessionId || !reportFormat || isGeneratingReport) ? "not-allowed" : "pointer", opacity: (!sessionId || !reportFormat || isGeneratingReport) ? 0.6 : 1 }}
          >
