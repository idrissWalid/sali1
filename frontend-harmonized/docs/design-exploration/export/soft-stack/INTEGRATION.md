...
Trois volumes arrondis qui s'empilent : le jeu de données, le modèle, le résultat publié.
...
2. **Fichier statique** — `<img src="/brand/soft-stack/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Chaque forme se pose avec un léger dépassement — cercle, puis pilule, puis carré d'accent.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-settle">…</svg>
...
```text
Intègre le logo SALI AI (variante 2C « Soft stack ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, soft-stack/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de soft-stack/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : soft-stack/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
