...
Trois lignes de tableur fragmentées se réduisent à une seule barre pleine : la conclusion publiée.

- **Type** : symbole + wordmark
- **Police du wordmark** : Barlow Condensed
...
2. **Fichier statique** — `<img src="/brand/resolve/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Les trois lignes brutes apparaissent en cascade (60 ms d'écart), puis la barre d'accent se déploie de gauche à droite. En état « analyse en cours », la barre balaye en aller-retour.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-resolve">…</svg>
...
```text
Intègre le logo SALI AI (variante 1B « Resolve ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, resolve/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de resolve/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : resolve/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
7. Charger la police Barlow Condensed (Google Fonts) et vérifier que le wordmark ne se coupe pas
