        ...
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 18 }}>
            ...
            <XAxis
              ...
              label={{ value: labelX, position: "insideBottom", offset: -10, fill: "var(--text-muted)", fontSize: 11 }}
            />
            <YAxis
              type="number" domain={[0, 1]}
              tick={{ fill: "var(--text-muted)", fontSize: 11 }} width={44}
              label={{ value: labelY, angle: -90, position: "insideLeft", fill: "var(--text-muted)", fontSize: 11 }}
            />
            <Tooltip
            ...
            {reference === "diagonale" ? (
              <Line
                type="linear" dataKey="x" stroke="var(--viz-context)" strokeDasharray="4 4"
                strokeWidth={1} dot={false} name="Hasard" isAnimationActive={false}
              />
            ) : typeof reference === "number" ? (
              <ReferenceLine
                y={reference} stroke="var(--viz-context)" strokeDasharray="4 4"
                label={{ value: labelRef, fill: "var(--viz-context)", fontSize: 10, position: "insideBottomRight" }}
              />
            ) : null}
            <Line
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {(retenu.roc_curve?.length ?? 0) > 0 && (
              <Courbe
                ...
                couleur="var(--viz-1)" sombre={theme === "dark"} delay={400}
              />
            )}
            {(retenu.pr_curve?.length ?? 0) > 0 && (
              <Courbe
                ...
                couleur="var(--viz-7)" sombre={theme === "dark"} delay={430}
              />
            )}
          </div>
        )}
