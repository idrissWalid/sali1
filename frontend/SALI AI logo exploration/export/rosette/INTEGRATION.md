...
Cinq pétales en boucle autour d'un centre : autant de familles de modèles convergeant vers un résultat.
...
2. **Fichier statique** — `<img src="/brand/rosette/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Les pétales apparaissent en éventail (72° d'écart), puis la rosette tourne lentement tant que l'agent travaille.
...
```text
Intègre le logo SALI AI (variante 3B « Rosette ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, rosette/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de rosette/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : rosette/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
