...
Un seul ruban arrondi dessine le S et change de couleur à mi-parcours : brut à l'aller, résolu au retour.
...
2. **Fichier statique** — `<img src="/brand/ribbon-s/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Le trait se dessine en deux temps (stroke-dashoffset) : moitié neutre, puis moitié accent. Bouclable en indicateur de progression.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-draw">…</svg>
...
```text
Intègre le logo SALI AI (variante 2B « Ribbon S ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, ribbon-s/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de ribbon-s/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : ribbon-s/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
