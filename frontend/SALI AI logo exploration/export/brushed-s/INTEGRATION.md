...
Le S sculpté en acier brossé, lame sombre derrière, éclairé en haut à gauche. Il lit comme un objet, pas comme une illustration.
...
2. **Fichier statique** — `<img src="/brand/brushed-s/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Un reflet balaye la surface en diagonale toutes les 4 s ; à l'entrée, la lame arrière pivote de 8° pour se caler.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-sheen">…</svg>
...
```text
Intègre le logo SALI AI (variante 5A « Brushed S ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, brushed-s/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de brushed-s/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : brushed-s/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
