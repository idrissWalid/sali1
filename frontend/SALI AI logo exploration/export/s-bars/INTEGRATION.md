...
Un S entièrement construit en barres de graphique : la lettre et la donnée sont le même objet.
...
2. **Fichier statique** — `<img src="/brand/s-bars/mark-dark-4a.svg" alt="SALI AI" width="32" height="32">`. Pratique pour un favicon, un e-mail ou un export bureautique ; aucune animation, et il faut choisir le fichier correspondant au mode.
...
Les cinq barres entrent en alternance par la gauche et la droite ; la barre d'accent en dernier.

```html
<!-- à l'entrée : la classe suffit -->
<svg class="sali-mark sali-anim-sbars">…</svg>
...
```text
Intègre le logo SALI AI (variante 1C « S-bars ») dans l'application.

Contexte : frontend Next.js, thème sombre par défaut (fond #131314, texte #e3e3e3),
un thème clair doit aussi être supporté.

À faire :
1. Copier le dossier export/ dans public/brand/ (tokens.css, animations.css, s-bars/).
...
4. Créer un composant <SaliLogo size="sm|md|lg" animated working /> qui rend le balisage
   de s-bars/inline.html. Ne pas remplacer les couleurs par des valeurs en dur :
...
6. Favicon : s-bars/mark-dark-4a.svg — vérifier la lisibilité à 16 px.
